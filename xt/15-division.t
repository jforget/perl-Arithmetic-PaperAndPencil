...
my $refcsv  = slurp('xt/data/15-division.csv' );
my $refhtml = slurp('xt/data/15-division.html');
...
$result = $operation->division(dividend => $x, divisor => $one, mult_and_sub => 'separate');
is($result->value, '9212', "division by 1 is obvious");
$result = $operation->division(dividend => $y, divisor => $x, mult_and_sub => 'separate');
is($result->value, '0', "division of a small dividend by a large divisor is obvious");
$result = $operation->division(dividend => $x, divisor => $y, result => 'remainder', mult_and_sub => 'separate');
